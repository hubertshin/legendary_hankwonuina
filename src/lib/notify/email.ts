 ...
 * 운영자에게 신청 알림 메일을 보낸다.
 *
 * 휴대폰 메일앱이 푸시를 띄워주므로 별도 비용 없이 즉시 알림을 받는다.
 * 알림톡과 달리 템플릿 심사나 대행사 계약이 필요 없다.
 *
 * 대상은 운영자 한 명(또는 몇 명)이라 발송량이 적어 Gmail SMTP로 충분하다.
 * 고객에게 보내는 확인 메시지는 이 경로로 할 수 없다 — 그건 알림톡이 필요하다.
 */

/** 알림 받을 주소. 쉼표로 여러 명 지정 가능. */
